...
void read_globals(FILE *file, Symbols_Table *table) {
    char line[256]; // Taille maximale d'une ligne dans le fichier source
    while (fgets(line, sizeof(line), file) != NULL) {
        // Ignorer les lignes vides
        if (strlen(line) <= 1) {
            continue;
        }
        // Ignorer les déclarations de fonctions
        if (strstr(line, "int ") != NULL && strstr(line, "(") != NULL) {
            continue;
        }
        // Ignorer la déclaration de la fonction main
        ...
        if (identifier != NULL && type != NULL) {
            ...
            symbol->adresse = 0; // Vous pouvez affecter une valeur appropriée à cette adresse
            symbol->deplct = 0; // Vous pouvez affecter une valeur appropriée à ce déplacement
